...
bool BTree::search_data(BTNode *btn, bitmask_t key, float **pdata){
	int numOfentry = btn->num_entries;
	...
	memcpy(*pdata, dnode->entries[mid].data, DIMENSION * sizeof(float));		//为啥这个版本一开始忘乘sizeof(float)了呢？
	return true;
}

bool BTree::search(bitmask_t key, float **pdata){
	bool flag = true;
	load_root();
